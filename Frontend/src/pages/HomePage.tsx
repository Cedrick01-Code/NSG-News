...
import { articles, categories, liveEvents } from '../data/mockData';
...
function LiveBanner() {
  ...
  return (
    <div
      className="rounded-2xl overflow-hidden mb-8 cursor-pointer relative"
      onClick={() => navigate('live')}
      style={{ border: '2px solid #CC1219' }}
    >
      <div className="relative h-40 sm:h-48">
        <img src={live.imageUrl} alt={live.title} className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/80 to-black/40 flex flex-col justify-center px-6">
          <div className="flex items-center gap-3 mb-2">
            <span className="flex items-center gap-1.5 px-3 py-1 bg-nsg-red rounded-full text-white text-xs font-bold uppercase tracking-wider">
              <span className="live-dot w-2 h-2 rounded-full bg-white inline-block" />
              Live
            </span>
            <span className="text-white/60 text-sm font-mono-nsg">{(live.viewers / 1000).toFixed(1)}K watching</span>
          </div>
          <h3 className="font-display text-white text-xl font-bold leading-snug">{live.title}</h3>
          <p className="text-white/70 text-sm mt-1">{live.location}</p>
        </div>
      </div>
    </div>
  );
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <aside className="space-y-8">
                ...
                {/* Premium promo */}
                <div
                  className="rounded-2xl p-5 text-center cursor-pointer hover:opacity-90 transition-opacity"
                  style={{ background: 'linear-gradient(135deg, #1B2A4A 0%, #2D4070 100%)' }}
                  onClick={() => navigate('pricing')}
                >
                  <div className="text-3xl mb-2">★</div>
                  <div className="font-display text-lg font-bold text-white mb-1">NSG Premium</div>
                  <p className="text-white/60 text-sm mb-4">Exclusive investigations, ad-free reading, and early access.</p>
                  <button className="px-5 py-2.5 bg-nsg-red text-white text-sm font-bold rounded-xl w-full hover:bg-nsg-red-dark transition-colors">
                    Explore Plans
                  </button>
                </div>
              </aside>
